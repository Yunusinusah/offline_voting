...
export function LiveMonitoring() {
  ...
  // Election status with memoization
  const electionStatus = useMemo(() => getElectionStatus(electionMeta), [electionMeta]);

  // Fetch monitoring data with better error handling
  const fetchMonitoring = useCallback(async (isRetry = false) => {
    try {
      ...
      // Apply data safely with validation
      if (data?.election) {
        setElectionMeta(data.election);
        // Calculate server-client offset for accurate countdown
        if (data.election.server_time) {
          try {
            const serverTime = new Date(data.election.server_time);
            const clientTime = new Date();
            serverClientOffsetRef.current = serverTime.getTime() - clientTime.getTime();
          } catch {
      ...
      if (data?.stats) {
        ...
        const validatedStats = {
          totalVoters: Math.max(0, Number(data.stats.totalVoters) || 0),
          votesCast: Math.max(0, Number(data.stats.votesCast) || 0),
          participationRate: Math.max(0, Math.min(100, Number(data.stats.participationRate) || 0)),
        };
        setStats(validatedStats);
      }

      // Handle voting trend data
      if (Array.isArray(data?.votingTrend)) {
        const validatedTrend = data.votingTrend
          .filter(item => item && typeof item === 'object')
          .map(item => ({
            time: item.time || new Date().toLocaleTimeString(),
            votes: Math.max(0, Number(item.votes) || 0)
          }));
        setVotingTrend(validatedTrend);
      }
      ...
      if (Array.isArray(data?.portfolios)) {
        const mapped = data.portfolios.map((p) => ({
          ...
          totalVotes: Math.max(0, Number(p.totalVotes) || 0),
          decisionTarget: p.decisionTarget, 
      ...
      if (isRetry && retryCount < 3) {
        ...
        setTimeout(() => fetchMonitoring(true), delay);
      }
    } finally {
      setIsLoading(false);
    }
  ...
  // Time remaining countdown with better accuracy
  useEffect(() => {
    if (tickRef.current) {
    ...
    // Update every second
    tickRef.current = setInterval(updateRemaining, 60000);
  ...
  // Format remaining time with better display
  ...
  useEffect(() => {
    ...
    if (isConnected && socket) {
      ...
      const onError = (error) => {
        ...
        // Fall back to polling on socket errors
        if (!pollInterval) {
          pollInterval = setInterval(() => fetchMonitoring(), 15000);
        }
      };
      ...
      fetchMonitoring();
      pollInterval = setInterval(() => fetchMonitoring(), 10000);
    }

    return () => {
      if (pollInterval) clearInterval(pollInterval);
    };
  }, [fetchMonitoring, isConnected, socket]);
  ...
  // Get countdown color and urgency
  ...
  // Render election inactive state
